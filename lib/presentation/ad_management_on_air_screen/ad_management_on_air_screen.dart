...
import 'bloc/ad_management_on_air_bloc.dart';
import 'models/ad_management_on_air_model.dart';

class AdManagementOnAirScreen extends StatelessWidget {
  const AdManagementOnAirScreen({Key? key}) : super(key: key);

  static Widget builder(BuildContext context) {
    return BlocProvider<AdManagementOnAirBloc>(
      create: (context) => AdManagementOnAirBloc(AdManagementOnAirState(
        adManagementOnAirModelObj: AdManagementOnAirModel(),
      ))..add(AdManagementOnAirInitialEvent()),
      child: AdManagementOnAirScreen(),
    );
  }
  ...
  Widget _buildRowsearch(BuildContext context) {
    return SizedBox(
      ...
      child: Row(
        ...
        children: [
          Expanded(
            child: BlocSelector<AdManagementOnAirBloc, AdManagementOnAirState, TextEditingController?>(
  ...
  Widget _buildFormtextfill(BuildContext context) {
    return BlocSelector<AdManagementOnAirBloc, AdManagementOnAirState, TextEditingController?>(
  ...
  /// current [adManagementOnAirModelObj] object if the user selects a valid date.
  ///
  /// This function returns a `Future` that completes with `void`.
  Future<void> onTapDate(BuildContext context) async {
    var initialState = BlocProvider.of<AdManagementOnAirBloc>(context).state;
    ...
    if (dateTime != null) {
      context.read<AdManagementOnAirBloc>().add(ChangeDateEvent(date: dateTime));
